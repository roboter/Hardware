/**************************************************
功能：本程序适用nRF24L01+的发送，当未按下按键时发送字符串"===Waiting!!!==="；
	  当按下按键时发送字符串"Starting to Send"
时间：2012年7月12日
/**************************************************/
...
uchar TX_BUF[TX_PLOAD_WIDTH]="===Waiting!!!===";
uchar TX_BUF1[TX_PLOAD_WIDTH]="Starting to Send"; 
//uchar buf[TX_PLOAD_WIDTH]="Data_changes!   ";
uchar flag;
uchar k,num=0;
uchar bdata sta;
sbit  RX_DR	 = sta^6;
...
void main(void)
{
	...
	while(1)
	{
		...
		{
			while(key1==0);
			for(num=0;num<16;num++) TX_BUF[num]=TX_BUF1[num];//改变发送数据 
		}
		TX_Mode(TX_BUF);		              // 设置为接收模式
		while(CheckACK());//检测应答信号(清空FIFO ,清除TX_DS或MAX_RT中断标志)
		delay_ms(100);
		//delay_ms(250);
