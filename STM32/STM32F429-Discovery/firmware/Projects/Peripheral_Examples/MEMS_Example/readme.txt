...
STM32F429I-DISCO board either in interrupt mode or in polling mode.
 
First, the L3GD20 MEMS is configured in interrupt mode on INT1 line. If no data
is present in MEMS fifo, no interrupt is generated then LED4(red) is On. Once
board moves, an interrupt is generated on INT1 line (EXTI line1) the LED3(green)
is On and LED4 is Off.
The program will wait for the user button to be pressed so the gyroscope angular 
data rate is read infinitely. According to the angle value, an arrow will be 
displayed on LCD.
