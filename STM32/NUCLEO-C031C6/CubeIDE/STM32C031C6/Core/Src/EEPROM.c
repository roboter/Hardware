...
#define PAGE_SIZE (2048/8) // in Bytes
#define PAGE_NUM  8    // number of pages
#define I2C_MEMADD_SIZE I2C_MEMADD_SIZE_16BIT // I2C_MEMADD_SIZE_8BIT
//Byte write cycle (BR24L01A/02/04/08/16-W) 8 bit
//Byte write cycle (BR24L32/64-              16 bit
...
void EEPROM_Write(uint16_t page, uint16_t offset, uint8_t *data, uint16_t size) {
	...
	int paddrposition = log(PAGE_SIZE) / log(I2C_MEMADD_SIZE);
	...
	for (int i = 0; i < numofpages; i++) {
		...
		HAL_I2C_Mem_Write(EEPROM_I2C, EEPROM_ADDR, MemAddress, I2C_MEMADD_SIZE,
				&data[pos], bytesremaining, 1000); // write the data to the EEPROM
...
void EEPROM_Read(uint16_t page, uint16_t offset, uint8_t *data, uint16_t size) {
	...
	for (int i = 0; i < numofpages; i++) {
		...
		HAL_I2C_Mem_Read(EEPROM_I2C, EEPROM_ADDR, MemAddress, I2C_MEMADD_SIZE,
				&data[pos], bytesremaining, 1000);
		startPage += 1;
		offset = 0;
