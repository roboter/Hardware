...
{	 
	...
	{
		VS_WR_Cmd(SPI_MODE,0x0804);// 软件复位,新模式	软复位成功后 第三位会从1跳0 这时说明复位成功    
		delay_ms(3);//等待至少1.35ms 
		if(retry++>100)break; 	    
	}	 		 
	while(VS_DQ==0);//等待软件复位结束	 
	retry=0;
...
//	U8 bands;
	...
	VS_WR_Cmd(SPI_WRAMADDR,SPEC_dat_BASE+4);  //写读取频谱命令
	for (i=0;i<14;i++) 
	{  
...
{
	u8 i;
	VS_WR_Cmd(SPI_WRAMADDR,SPEC_dat_BASE+0X58);//地址总是1868,对VS1053,SPEC_dat_BASE是0X1810.所以加上0X58  
	for (i=0;i<bands;i++) 
	{
		VS_WR_Cmd(SPI_WRAM,buf[i]);//发送频率数据  
	}
	if(i<15)VS_WR_Cmd(SPI_WRAM,25000);//对VS1053这个最大值为15,填充25000,表示频谱表结束
 	VS_WR_Cmd(SPI_WRAMADDR,SPEC_dat_BASE+1);//地址SPEC_dat_BASE+1,为Samples Rates的起始地址
