...
{  
	...
	TIM_ICInitTypeDef  TIM_ICInitStructure;  			 //定时器输入捕获结构体定义
 
 	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOA,ENABLE); //使能PORTB时钟 
	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2,ENABLE);	 //TIM2 时钟使能 

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_3;		     //PA3 输入 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; 		 //上拉输入 
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOA, &GPIO_InitStructure);
 	GPIO_SetBits(GPIOA,GPIO_Pin_3);	    //初始化GPIOA.3	  置一状态
	
						  
 	TIM_TimeBaseStructure.TIM_Period = 10000;       //设定计数器自动重装值 1us加1 10000=10ms 最大10ms溢出  
	TIM_TimeBaseStructure.TIM_Prescaler =(72-1); 	//预分频器,72000000/72=1000000=1M的计数频率,1us加1.	   
	...
	TIM_TimeBaseInit(TIM2, &TIM_TimeBaseStructure); //根据指定的参数初始化TIMx
  	...
  	TIM_ICInitStructure.TIM_Channel = TIM_Channel_4;               //选择输通道 IC4映射到TI4上  4通道
  	TIM_ICInitStructure.TIM_ICPolarity = TIM_ICPolarity_Rising;	   //上升沿捕获
  	TIM_ICInitStructure.TIM_ICSelection = TIM_ICSelection_DirectTI;//直接选择通道
