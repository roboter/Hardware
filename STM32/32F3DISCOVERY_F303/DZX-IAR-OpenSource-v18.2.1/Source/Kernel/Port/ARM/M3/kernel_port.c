...
void KERNEL_ExecutePort(THREADSTART proc, void* arg)
{
    ...
    proc(arg);                                                  /* Launch into the initial thread */
}
...
STATUS THREAD_InitContext(THREAD* thread, THREADSTART proc, void* arg)
{
    ...
    frame->R0 = (UINT32)arg;                                        /* Initialize the registers for the thread */
    frame->R1 = 0;
    frame->R2 = 0;
    frame->R3 = 0;
    frame->R12 = 0;
...
/***************************************************************************************************************************************//**
    ...
    The PendSV exception handler. Performs a thread context switch.
...
__asm void PendSV_Handler(void)
{
    ...
    MRS     R0, PSP                     ; Store the kernel managed registers onto the current threads stack
    SUBS    R0, R0, #0x20
    STM     R0, {R4-R11}
    ...
    LDM     R0, {R4-R11}                ; Restore the kernel managed registers from the next threads stack
    ADDS    R0, R0, #0x20
    MSR     PSP, R0
