/*******************************************************************************************************************************************
    ...
    DZX | LPC18xx | FLASH                                                                                                 VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
...
/***************************************************************************************************************************************//**
    ...
    @module     flash-lpc18xx

*******************************************************************************************************************************************/
UINT32 FLASH_GetSector(UINT32 address)
{
    if (address < 0x1A010000) {
        return ((address - 0x1A000000) / 0x2000);               /* The address is within the 8KB sectors of bank A */
    }
    
    if (address < 0x1A080000) {
        return ((address - 0x1A010000) / 0x10000) + 8;          /* The address is within the 64KB sectors of bank A */
    }
    
    if (address < 0x1B010000) {
        return ((address - 0x1B000000) / 0x2000);               /* The address is within the 8KB sectors of bank B */
    }
    
    return ((address - 0x1B010000) / 0x10000) + 8;              /* The address is within the 64KB sectors of bank B */
}
...
*******************************************************************************************************************************************/
UINT32 FLASH_GetBank(UINT32 address)
{
    if (address < 0x1B000000) {
        return 0;
    }
...
{
    if (address < 0x1A010000) {                                 /* Is the address within 8KB sectors of bank A */
        if ((address % 0x2000) == 0) {
            return TRUE;                                        /* Yes, address is the start of a 8KB sector */
        }
    } else if (address < 0x1B000000) {                          /* Is the address within 64KB sectors of bank A */
        if ((address % 0x10000) == 0) {
            return TRUE;                                        /* Yes, address is the start of a 64KB sector */
        }
    } else if (address < 0x1B010000) {                          /* Is the address within 8KB sectors of bank B */
        if ((address % 0x2000) == 0) {
            return TRUE;                                        /* Yes, address is the start of a 8KB sector */
        }
    } else {
...
*******************************************************************************************************************************************/
STATUS FLASH_EraseSector(UINT32 sector, UINT32 bank)
{
    STATUS status;

    
    status = IAP_PrepareSector(sector, sector, bank);           /* Prepare the sector to be erased */
    if (status == SUCCESS) {
        status = IAP_EraseSector(sector, sector, bank);         /* Erase the sector */
    }
...
STATUS FLASH_Write(UINT32 addr, const void* data, UINT32 nbytes)
{
    ...
    bank = FLASH_GetBank(addr);                                             /* Lookup the bank */
    
    status = IAP_PrepareSector(sector, sector, bank);                       /* Prepare the sector */
    if (status == SUCCESS) {
        status = IAP_CopyRamToFlash(addr, (UINT32)data, nbytes);            /* Program the data */
    }
