/*******************************************************************************************************************************************
    ...
    DZX | LPC177x8x | SPI                                                                                                 VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
...
STATUS SPI_Open(SPI* port, UINT32 number, UINT32 clkHz, UINT32 mode)
{
    ...
    switch (number) {
        ...
        case 2:
            port->REG = REG_SSP2;
            ACQUIREINTERRUPTLOCK();
            REGBITS_PCONP.PCSSP2 = 1;
            RELEASEINTERRUPTLOCK();
            IRQ_RegisterHandler(IRQ_SSP2, SSP_IRQ);
            IRQ_Enable(IRQ_SSP2);
            break;
        default:
...
STATUS SPI_Close(SPI* port)
{
    ...
    switch (port->number) {                                     /* Find by port number */
        ...
        case 1:
            ...
            break;
        case 2:
            IRQ_Disable(IRQ_SSP2);                             /* Disable possible interrupts */
            REGBITS_PCONP.PCSSP2 = 0;                           /* Power down the peripheral */
            break;
        default:
            RELEASEINTERRUPTLOCK();
...
static void SPI_ApplyClock(SPI* port, UINT32 baud)
{
    ...
    prescalar = 2;
    divisor = (CFG_CPUCLOCKHZ / (baud * prescalar * REGBITS_PCLKSEL.PCLKDIV)) - 1;
...
/***************************************************************************************************************************************//**
    ...
    Sets a new clock rate for a SPI port.

    @param[in]  port        A pointer to the target port.
    @param[in]  baud        The new clock rate, in hertz, for the port.

    @return

    Status Code            | Description
    ----------------------- | ------------------------------------------------------------------------------------------------------
     SUCCESS                | The new clock rate has been set.
     ERR_NULLREFERENCE      | The argument 'port' was found to be NULL.

    @module     spi-lpc177x8x

*******************************************************************************************************************************************/
STATUS SPI_SetClock(SPI* port, UINT32 clkHz)
{
#if (CFG_NULLREFERENCE)
    if (port == NULL) {
        return ERR_NULLREFERENCE;
    }
#endif
    
    SPI_ApplyClock(port, clkHz);
    return SUCCESS;
}
