/*******************************************************************************************************************************************
    ...
    DZX | LPC43xx | KERNEL                                                                                                VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
...
static void MCU_InitClock(void)
{
    ...
    REGBITS_CGUBASEM4CLK.CLKSEL = 0xE;
    ...
    for (k = 0; k < 12000; k++);                                /* Wait for the oscillator to stabilize (250uS user manual) */
    
    REGBITS_CGUBASEM4CLK.AUTOBLOCK = 1;
    REGBITS_CGUBASEM4CLK.CLKSEL = 6;
    ...
    REGBITS_CGUPLL1CTRL.DIRECT = 0;                             /* Don't allow direct connection yet, step up */
    
    REGBITS_CGUPLL1CTRL.MSEL = CFG_PLL1MSEL;                    /* Assign multipliers and dividers */
    ...
    while (REGBITS_CGUPLL1STAT.LOCK == 0);                      /* Wait for pll to lock */
    
    REGBITS_CGUBASEM4CLK.CLKSEL = 9;                            /* Select the pll as the main clock source */
    REGBITS_CGUBASEAPB1CLK.CLKSEL = 9;
    REGBITS_CGUBASEAPB3CLK.CLKSEL = 9;
    
    for (k = 0; k < 500; k++);                                  /* Wait for 50us per spec before switching to direct (high speed) */
    REGBITS_CGUPLL1CTRL.DIRECT = CFG_PLL1DIRECT;                /* Switch to direct */
