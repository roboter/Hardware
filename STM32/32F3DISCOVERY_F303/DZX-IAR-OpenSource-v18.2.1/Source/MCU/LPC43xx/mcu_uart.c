/*******************************************************************************************************************************************
    ...
    DZX | LPC43xx | UART                                                                                                  VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
...
/*******************************************************************************************************************************************
    PRIVATE VARIABLES
*******************************************************************************************************************************************/
static UART* mcu_uart[4];                           /* Pointers to each of the open UARTs */
...
/***************************************************************************************************************************************//**
    ...
    Opens a UART for communications.

    @param[in]  uart        A pointer to a caller allocated uart instance to be opened.
    @param[in]  number      The port number to be opened.
    ...
    ----------------------- | ------------------------------------------------------------------------------------------------------
     SUCCESS                | The UART has been initialized and is open for communications.
...
STATUS UART_Open(UART* uart, UINT32 number, UINT32 baud, UINT32 nbits, STOPBITS stopbits, PARITY parity, HANDSHAKE hs)
{
    ...
    if (number >= 5) {                                          /* A valid port number? */
    ...
    switch (number) {                                           /* Power up the UART and set peripheral clock frequency */
        case 0:
            uart->REG = REG_UART0;
            REGBITS_CGUBASEUART0CLK.AUTOBLOCK = 1;
            REGBITS_CGUBASEUART0CLK.CLKSEL = 0x9;
            REGBITS_CGUBASEUART0CLK.PD = 0;
            IRQ_RegisterHandler(IRQ_USART0, UART_IRQ);
            IRQ_Enable(IRQ_USART0);
            break;
        case 1:
            uart->REG = REG_UART1;
            REGBITS_CGUBASEUART1CLK.AUTOBLOCK = 1;
            REGBITS_CGUBASEUART1CLK.CLKSEL = 0x9;
            REGBITS_CGUBASEUART1CLK.PD = 0;
            IRQ_RegisterHandler(IRQ_UART1, UART_IRQ);
            IRQ_Enable(IRQ_UART1);
            break;
        case 2:
            uart->REG = REG_UART2;
            REGBITS_CGUBASEUART2CLK.AUTOBLOCK = 1;
            REGBITS_CGUBASEUART2CLK.CLKSEL = 0x9;
            REGBITS_CGUBASEUART2CLK.PD = 0;
            IRQ_RegisterHandler(IRQ_USART2, UART_IRQ);
            IRQ_Enable(IRQ_USART2);
            break;
        case 3:
            uart->REG = REG_UART3;
            REGBITS_CGUBASEUART3CLK.AUTOBLOCK = 1;
            REGBITS_CGUBASEUART3CLK.CLKSEL = 0x9;
            REGBITS_CGUBASEUART3CLK.PD = 0;
            IRQ_RegisterHandler(IRQ_USART3, UART_IRQ);
            IRQ_Enable(IRQ_USART3);
            break;
        default:
    ...
    uart->REG->FCR.BITS.FIFOEN = 1;                             /* Enable FIFO's for the uart */
    uart->REG->FCR.BITS.RXTRIGLVL = 2;                          /* Receive interrupt level at 8 characters */
    uart->REG->IER.BITS.RBRIE = 1;                              /* Enable the receive interrupt */
...
STATUS UART_Close(UART* uart)
{
    ...
    switch (uart->number) {
        case 0:
            IRQ_Disable(IRQ_USART0);        /* Disable the NVIC interrupt */
            break;
        case 1:
            IRQ_Disable(IRQ_UART1);         /* Disable the NVIC interrupt */
            break;
        case 2:
            IRQ_Disable(IRQ_USART2);        /* Disable the NVIC interrupt */
            break;
        case 3:
            IRQ_Disable(IRQ_USART3);        /* Disable the NVIC interrupt */
            break;
    }
...
static void UART_SetBaud(UART* uart)
{
    ...
    temp = (UINT64)(CFG_CPUCLOCKHZ) * UARTBAUDPRECISION;
    
    base = (UINT32)(temp / (uart->baud * 16));              /* Calculate a base divisor */
...
STATUS UART_Clear(UART* uart)
{
    ...
    ACQUIREINTERRUPTLOCK();
    uart->REG->FCR.BITS.RXFIFORES = 1;                          /* Empty the hardware receive FIFO */
    RELEASEINTERRUPTLOCK();
...
/***************************************************************************************************************************************//**
    ...
    @param[in]  uart        A pointer to the target port.
...
/*******************************************************************************************************************************************
    ...
    @param[in]  uart        A pointer to the UART that has had an interrupt request.
...
static void UART_IRQ(IRQ irq)
{
    ...
    switch (uart->REG->IIR.BITS.INTID) {                                /* Determine the source of the interrupt */
