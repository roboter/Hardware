...
STATUS SPI_Open(SPI* port, UINT32 number, UINT32 clkHz, UINT32 mode)
{
    ...
    switch (number) {
        ...
        case 4:
            ...
            ACQUIREINTERRUPTLOCK();
            REGBITS_RCCAPB2ENR.SPI4EN = 1;
            RELEASEINTERRUPTLOCK();
            
            IRQ_RegisterHandler(IRQ_SPI4, SPI_IRQ);
            IRQ_Enable(IRQ_SPI4);
            break;
        case 5:
            port->REG = REG_SPI5;
            ...
            ACQUIREINTERRUPTLOCK();
            REGBITS_RCCAPB2ENR.SPI5EN = 1;
            RELEASEINTERRUPTLOCK();
            
            IRQ_RegisterHandler(IRQ_SPI5, SPI_IRQ);
            IRQ_Enable(IRQ_SPI5);
            break;
        default:
...
STATUS SPI_Close(SPI* port)
{
    ...
    switch (port->number) {                                 /* Find by port number */
        ...
        case 3:
            ...
            break;
        case 4:
            IRQ_Disable(IRQ_SPI4);                          /* Disable possible interrupts */
            break;
        case 5:
            IRQ_Disable(IRQ_SPI5);                          /* Disable possible interrupts */
            break;
        default:
            RELEASEINTERRUPTLOCK();
...
STATUS SPI_Write(SPI* port, const void* data, UINT32 nbytes, UINT32 timeout)
{
    ...
    port->REG->DR = ((BYTE*)port->data)[0];                             /* Initiate the first transfer */
...
STATUS SPI_Read(SPI* port, void* buf, UINT32 nbytes, UINT32 timeout)
{
    ...
    port->REG->DR = 0;                                                      /* Initiate a transfer */
...
static void SPI_IRQ(IRQ irq)
{
    ...
    if (port->buf) {
        ((BYTE*)port->buf)[port->idx] = port->REG->DR;          /* Read the received data byte */
    } else {
        dummy = port->REG->DR;
    }
