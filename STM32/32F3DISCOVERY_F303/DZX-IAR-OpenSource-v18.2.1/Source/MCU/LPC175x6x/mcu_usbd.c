...
STATUS USBD_InitController(USBDDEVICE* device)
{
    ...
    REG_USBCLKCTRL = 0x12;                                      /* Enable the clocks within the USB peripheral */
    while ((REG_USBCLKCTRL & 0x12) != 0x12) { }                 /* Wait for the clocks to become valid */
...
static void USBD_InitPLL(void)
{
    if (REGBITS_PLL1STAT.PLLC) {                                /* Check if the PLL is currently connected and should be disconnected */
        REGBITS_PLL1CON.PLLC = 0;                               /* Set command to disconnect the PLL */
        USBD_FeedPLL();
    }

    if (REGBITS_PLL1STAT.PLLE) {                                /* Check if the PLL is currently enabled and should be disabled */
    ...
    REGBITS_PLL1CON.PLLC = 1;                                   /* Connect the PLL */
    USBD_FeedPLL();
}
