/*******************************************************************************************************************************************
    ...
    This is part of the MCU peripheral drivers and provides support for the SPI ports.
...
*******************************************************************************************************************************************/
#define REG_SPI0        ((REG_SPI*)0x40020000)
#define REG_SSP0        ((REG_SSP*)0x40088000)
#define REG_SSP1        ((REG_SSP*)0x40030000)

typedef struct S0SPCRBITFIELD {
    UINT32                         :2;
    UINT32 BITENABLE               :1;
    UINT32 CPHA                    :1;
    UINT32 CPOL                    :1;
...
typedef struct S0SPCCRBITFIELD {
    UINT32 COUNTER                 :8;
    UINT32                         :24;
} S0SPCCRBITFIELD;

typedef struct S0SPINTBITFIELD {
    UINT32 SPIINT                  :1;
    UINT32                         :31;
} S0SPINTBITFIELD;
...
typedef struct REG_SPI {
	PREG_BITS32(SPCR, S0SPCRBITFIELD);
	PREG_BITS32(SPSR, S0SPSRBITFIELD);
	PREG_32(SPDR);
	PREG_BITS32(SPCCR, S0SPCCRBITFIELD);
	volatile UINT32 reserved[3];
	PREG_BITS32(SPINT, S0SPINTBITFIELD);
} REG_SPI;
...
/*******************************************************************************************************************************************
    TYPE DEFINITIONS
*******************************************************************************************************************************************/
typedef struct LPC175X6X_SPI SPI;
...
*******************************************************************************************************************************************/
struct LPC175X6X_SPI {
	UINT32      number;         /**< The port number */
    UINT32      mode;
    UINT32      clkHz;
    REG_SPI*    SPIREG;         /**< A pointer to the SPI peripheral registers for the port */
    REG_SSP*    SSPREG;         /**< A pointer to the SSP peripheral registers for the port */
    MUTEX       mutex;          /**< A mutex to acquire exclusive access to the port */
    SIGNALSET   signals;        /**< A set of signals used to block/release threads using the port */
    void*       data;
    void*       buf;
    UINT32      idx;
    UINT32      nbytes;
