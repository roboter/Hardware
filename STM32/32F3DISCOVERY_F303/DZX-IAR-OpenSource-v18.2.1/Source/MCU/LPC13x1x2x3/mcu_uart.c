...
STATUS UART_Open(UART* uart, UINT32 number, UINT32 baud, UINT32 nbits, STOPBITS stopbits, PARITY parity, HANDSHAKE hs)
{
    ...
    if (number >= 1) {                                          /* A valid port number? */
    ...
    switch (number) {                                           /* Power up the UART and set peripheral clock frequency */
        case 0:
            uart->REG = REG_UART0;
            REGBITS_SYSAHBCLKCTRL.UART = 1;
            IRQ_RegisterHandler(IRQ_UART, UART_IRQ);
            IRQ_Enable(IRQ_UART);
            break;
        default:
...
STATUS UART_Close(UART* uart)
{
    ...
    switch (uart->number) {
        case 0:
            IRQ_Disable(IRQ_UART);                              /* Disable the NVIC interrupt */
            REGBITS_SYSAHBCLKCTRL.UART = 0;                     /* Power down the peripheral */
            break;
    }

    BUFFER_Destroy(&uart->rxb);
    BUFFER_Destroy(&uart->txb);
...
static void UART_SetBaud(UART* uart)
{
    ...
    base = (UINT32)(((UINT64)MCU_ClockHz() * UARTBAUDPRECISION) / (uart->baud * 16));      /* Calculate a base divisor */
