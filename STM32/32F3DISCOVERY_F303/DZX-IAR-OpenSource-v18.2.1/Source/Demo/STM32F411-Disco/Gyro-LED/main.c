/*******************************************************************************************************************************************
    ...
    DZX | STM32F411 DISCOVERY | GYRO LED                                                                                  VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
...
static void APP_Main(void* arg)
{
    ...
    for (;;) {
        ...
        if (status == SUCCESS) {
            ...
            LED_Set(LED_RED, sample.x < -threshold);            /* Turn on the LEDs based upon the gyro data */
            LED_Set(LED_GREEN, sample.x > threshold);
        
            LED_Set(LED_ORANGE, sample.y < -threshold);
            LED_Set(LED_BLUE, sample.y > threshold);
        }
