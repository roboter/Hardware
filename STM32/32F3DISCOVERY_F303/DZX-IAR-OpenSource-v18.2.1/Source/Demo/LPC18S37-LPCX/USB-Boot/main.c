/*******************************************************************************************************************************************
    ...
    DZX | LPC18S37-LPCX | USB-BOOT                                                                                        VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
...
const BYTE Key[] = {                                        /* AES Key (AES-128 with passphrase "password") */
...
static void APP_TryExecuteRuntime(void)
{
    ...
    if (REGBITS_ERSTATUS.WAKEUP0ST) {							/* If WAKE0 button is pressed, stay in bootloader */
        return;
    }
...
static void APP_Download(DFUDBLOCK* blk)
{
    ...
    if (FLASH_IsSectorStart(address)) {                                 /* Is the data targeted at a new sector? */
        FLASH_EraseSector(FLASH_GetSector(address), 
                          FLASH_GetBank(address));                      /* Erase the sector that is about to be programmed */
    }
...
static void APP_InitUSB(void)
{
    ...
    status = USBD_SetDeviceName(&usbd,                          /* Assign name strings for the device */
                                ...
                                "LPC18S37 BOOTLOADER",          /* Product Name */
                                NULL);                          /* Serial Number */
    ...
    status = DFUD_CreateInterface(&usbd,                        /* A pointer to the USB device stack */
                                  ...
                                  CFG_DFUDTRANSFERSIZE);        /* The size, in bytes, of the provided transfer buffer */
    assert(status == SUCCESS);
