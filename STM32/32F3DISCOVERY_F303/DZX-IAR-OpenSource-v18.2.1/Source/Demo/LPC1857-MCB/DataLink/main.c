/*******************************************************************************************************************************************
    ...
    DZX | LPC1857-MCB | DATALINK                                                                                          VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------

    This is an application that implements a demonstration data link on the NXP MCB1857 evaluation board. The application provides the 
    link over native USB and over UART 3 (FTDI connector).

    The data link definition is from demolink.xml that is found within the Microsoft.NET and Utility Tools download from dzxdesigns.com.
...
static void APP_Timer1(TIMER* timer, void* arg)
{
    ...
    if (TIMER_IsRunning(&timer2)) {                             /* Timer running indicates whether streaming device status */
        ...
        LED_Set(LED_0, FALSE);                                  /* Blink LED 7 while streaming device status */
        LED_Set(LED_7, state);
    } else {
        
        LED_Set(LED_0, state);                                  /* Blink LED 0 while streaming is disabled */
        LED_Set(LED_7, FALSE);
    }
}
...
static void APP_Main(void* arg)
{
    ...
    PIN_SetFunction(P2_03, 2);                          /* Assign P2.3 to the U3_TXD function */
    PIN_SetFunction(P2_04, 2);                          /* Assign P2.4 to the U3_RXD function */
    PIN_SetInputBuffer(P2_04, TRUE);                    /* Enable the input buffer for P2.4 */
