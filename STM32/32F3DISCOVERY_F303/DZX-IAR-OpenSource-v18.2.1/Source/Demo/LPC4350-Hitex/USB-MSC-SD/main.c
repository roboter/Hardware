/*******************************************************************************************************************************************
    ...
    DZX | LPC4350-HITEX | USB-MSC-SD                                                                                      VERSION 18.2.1
    ------------------------------------------------------------------------------------------------------------------------------------
    ...
    access to the on-board SD/MMC card.
...
static void APP_Main(void* arg)
{
    ...
    status = TIMER_Create(&timer,                               /* Create a timer for blinking LED */
                          ...
                          APP_Timer1,                           /* The callback expiration function */
                          NULL,                                 /* An optional argument */
                          200,                                  /* The timer interval, in kernel ticks */
                          OPT_START | OPT_AUTORESET);           /* Options, start the timer and auto-reset on expiration */
...
static void APP_Timer1(TIMER* timer, void* arg)
{
    ...
    if (DISK_Mounted(&disk.base)) {                             /* Blink the LEDs based upon the mount status of the SD card */
        ...
        LED_Set(LED_GREEN, state);                              /* Blink the green LED while the card is mounted */
        LED_Set(LED_RED, FALSE);
        
    } else {
        
        LED_Set(LED_GREEN, FALSE);                              /* Blink the red LED while the card is NOT mounted */
